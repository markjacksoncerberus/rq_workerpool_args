...
class WorkerPoolPerformance:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mem_target: int = mem_target
        self.starting_worker_count: int = starting_worker_count
    ...
    def calc_target_worker_count(self):
        
        cpu_usage = round(psutil.cpu_percent(interval=1), 2)
        mem_usage = round(psutil.virtual_memory().percent, 2)
        active_worker_count = len(self.worker_dict)

        # Surround in try except to prevent division by zero
        # Has only been seen to happen from active_worker_count being 0
        # But this is a safety measure
        try:
            average_worker_cpu_usage = round(cpu_usage / active_worker_count, 2) or 0.01
            average_worker_mem_usage = round(mem_usage / active_worker_count, 2) or 0.01
        except ZeroDivisionError:
            return self.starting_worker_count

        target_count_cpu = self.cpu_target / average_worker_cpu_usage
        target_count_mem = self.mem_target / average_worker_mem_usage

        print(f"AUTOSCALER | CPU usage: {cpu_usage}%, Memory usage: {mem_usage}% | Avg worker CPU: {average_worker_cpu_usage}%, Avg worker MEM: {average_worker_mem_usage}%")
        print(f"AUTOSCALER | {target_count_cpu=} | {target_count_mem=}")

        target_worker_count = round(max(self.starting_worker_count, min(target_count_cpu, target_count_mem)))
    ...
    def check_workers(self, respawn: bool = True) -> None:
        ...
        self.worker_dict = {name: data for name, data in self.worker_dict.items() if data.process.is_alive()}

        target_worker_count = self.calc_target_worker_count()
        # If we have less number of workers than starting_worker_count,
        # respawn the difference
        if respawn and self.status != self.Status.STOPPED:
            delta = target_worker_count - len(self.worker_dict)
            ...
            if delta < 0:
                ...
                for i in range(delta):
                    self.stop_worker(list(self.worker_dict.values())[i])
    ...
    def start_worker(
        ...
        * sleep: waits for X seconds before creating worker, for testing purposes
        ...
        process.start()
        worker_data = WorkerData(name=name, pid=process.pid, process=process, stop_attempt_count=0)  # type: ignore
    ...
    def start_workers(self, burst: bool = True, _sleep: float = 0, logging_level: str = "INFO"):
        ...
        * sleep: waits for X seconds before creating worker, only for testing purposes
        """
        self.log.debug(f'Spawning {self.starting_worker_count} workers')
    ...
    def stop_worker(self, worker_data: WorkerData, sig=signal.SIGINT):
        ...
        """
        try:
            # If a worker has already received sigint, don't send it again.
            # Let it complete the job safely.
            if worker_data.stop_attempt_count > 0:
                return
            else:
                worker_data.stop_attempt_count += 1
